...
exports.ShExUtil = exports.isShapeRef = exports.isTerm = exports.extend = void 0;
...
exports.ShExUtil = {
    ...
    getAST: function (schema) {
        return {
            type: "AST",
            shapes: schema.shapes?.reduce((ret, shape) => {
                ret[typeof shape === "string" ? shape : shape.id] = {
                    type: "ASTshape",
                    expression: typeof shape !== "string" && "expression" in shape
                        ? _compileShapeToAST(shape.expression, [], schema)
                        : shape,
                };
                return ret;
            }, {}),
            ...
            function _compileExpression(expr, schema) {
                ...
                */
                function _repeat(expr, min, max) {
                    ...
                    const reqd = min !== 0
                        ? new EachOf(Array.from(Array(min))
                            .map(function (ret) {
    ...
    ShExRVisitor: function (knownShapeExprs) {
        const v = exports.ShExUtil.Visitor();
        const knownExpressions = {};
        const oldVisitShapeExpr = v.visitShapeExpr, oldVisitExpression = v.visitExpression;
        ...
        if ("shapes" in schema) {
            knownShapeExprs = [
                ...
                ...(schema.shapes?.map((sh) => {
                    return typeof sh !== "string" && sh.id ? sh.id : sh;
                }) ?? []),
            ];
        }
    ...
    canonicalize: function (schema, trimIRI) {
        ...
        v.visitExpression = function (expression) {
            if (typeof expression === "object" && "id" in expression) {
                const shape = typeof expression !== "string"
                    ? expression.id
                    : expression;
    ...
    nestShapes: function (schema, options = {}) {
        ...
        let nestables = Object.keys(shapeReferences)
            .filter((label) => shapeReferences[label].length === 1 &&
        ...
        if (!options.no) {
            ...
            if (options.rename) {
                ...
                Object.keys(nestables).forEach((oldName) => {
                    ...
                    if (ref.type === "tc") {
                        if (typeof ref.tc.valueExpr === "string") {
                            ...
                            throw Error("assertion: rename not implemented for TripleConstraint expr: " +
                                ref.tc.valueExpr);
                ...
                const doomed = [];
                const ids = schema.shapes?.map((s) => (typeof s === "string" ? s : s.id)) ?? [];
                Object.keys(nestables).forEach((oldName) => {
                    ...
                    if (schema.shapes &&
                        ((typeof schema.shapes[delme] === "string" &&
                            ...
                            (typeof schema.shapes[delme] !== "string" &&
                                schema.shapes[delme].id !== oldName)))
                        throw Error("assertion: found " +
                            (typeof schema.shapes[delme] === "string"
                                ? schema.shapes[delme]
                                : schema.shapes[delme].id) +
                            " instead of " +
                            oldName);
                    doomed.push(delme);
                    delete index.shapeExprs[oldName];
                ...
                doomed
                    ...
                    .forEach((delme) => {
                    if (schema.shapes) {
                        const id = (typeof schema.shapes[delme] === "string"
                            ? schema.shapes[delme]
                            : schema.shapes[delme].id);
                        if (!nestables[id])
    ...
    getPredicateUsage: function (schema, untyped = {}) {
        ...
        Object.keys(schema.shapes).forEach((label, labelIndex) => {
            let shapeExpr = schema.shapes[labelIndex];
        ...
        Object.keys(schema.shapes).forEach((shapeLabel, shapeLabelIndex) => {
            let shapeExpr = schema.shapes[shapeLabelIndex];
            if (typeof shapeExpr !== "string" && shapeExpr.type === "Shape") {
                ...
                tcs.forEach((tc) => {
                    ...
                    if (!(tc.predicate in predicates)) {
                        ...
                        else if (shapeHierarchy.parents[curType] &&
    ...
    simpleTripleConstraints: function (shape) {
        ...
        if (typeof shape.expression !== "string" &&
            ...
            !shape.expression.expressions.find((expr) => typeof expr !== "string" && expr.type !== "TripleConstraint")) {
        ...
        (schema.shapes || []).forEach(function (shape) {
            function _walkShapeExpression(shapeExpr, negated) {
                if (typeof shapeExpr === "string") {
                    // ShapeRef
                    ret?.add(Number(typeof shape === "string" ? shape : shape.id), Number(shapeExpr));
                }
                    ...
                    throw Error("expected Shape{And,Or,Ref,External} or NodeConstraint in " +
            ...
            function _walkShape(shape, negated) {
                function _walkTripleExpression(tripleExpr, negated) {
                    ...
                    function _walkTripleConstraint(tc, negated) {
                        ...
                        if (negated &&
                            ret?.inCycle.indexOf((typeof shape === "string" ? shape : shape.id)) !== -1)
                            // illDefined/negatedRefCycle.err
                            throw Error("Structural error: " +
                                (typeof shape === "string" ? shape : shape.id) +
                                " appears in negated cycle");
                    }
                    if (typeof tripleExpr === "string") {
                        // Inclusion
                        ret?.add(Number(typeof shape === "string" ? shape : shape.id), Number(tripleExpr));
                    }
                    else {
                        if ("id" in tripleExpr)
                            ret?.addIn(tripleExpr.id, shape);
                        if (tripleExpr.type === "TripleConstraint") {
                            _walkTripleConstraint(tripleExpr, negated);
                        }
                        else if (tripleExpr.type === "OneOf" ||
                            tripleExpr.type === "EachOf") {
                            _exprGroup(tripleExpr.expressions, 0);
                        }
                        else {
                            throw Error("expected {TripleConstraint,OneOf,EachOf,Inclusion} in " +
                                tripleExpr);
                        }
                    }
                }
    ...
    partition: function (schema, includes, deps, cantFind) {
        ...
        const partition = {};
        for (let k in schema)
            partition[k] = k === "shapes" ? [] : schema.shapes;
    ...
    merge: function (left, right, overwrite, inPlace) {
        ...
        function mergeMap(attr) {
            (left[attr] || {}).forEach(function (_value, key, _map) {
                if (!(attr in ret))
                    ret[attr] = {};
                ret[attr].set(key, left[attr].get(key));
            });
            (right[attr] || {}).forEach(function (_value, key, _map) {
    ...
    validateSchema: function (schema) {
        ...
        visitor.visitShapeRef = function (shapeRef) {
            if (!(shapeRef in index.shapeExprs))
                throw firstError(Error("Structural error: reference to " +
        ...
        visitor.visitInclusion = function (inclusion) {
            ...
            if (!refd)
                throw firstError(Error("Structural error: included shape " +
                    inclusion +
        ...
        (schema.shapes || []).forEach(function (shape) {
            currentLabel = (typeof shape === "string" ? shape : shape.id);
            visitor.visitShapeExpr(shape, (typeof shape === "string" ? shape : shape.id));
        });
        let circs = Object.keys(negativeDeps.children).filter((k) => negativeDeps.children[k].filter((k2) => (k2 in negativeDeps.children &&
            negativeDeps.children[k2].indexOf(k) !== -1) ||
            (k2 in positiveDeps.children &&
                positiveDeps.children[k2].indexOf(k) !== -1)).length > 0);
        if (circs.length)
            throw firstError(Error("Structural error: circular negative dependencies on " +
                circs.join(",") +
                "."), circs[0]);
    ...
    walkVal: function (val, cb) {
        ...
        else if (val.type === "EachOfSolutions" ||
        ...
        function visitSolution(sln, vals) {
            ...
            function chaseList(li) {
                if (!li)
                    return false;
                if (li.node === namespaces_1.RDF.nil)
                    return true;
                if ("solution" in li &&
                    ...
                    li.solution.solutions[0].expressions[0]
                        .predicate === namespaces_1.RDF.first &&
                    typeof li.solution.solutions[0].expressions[1] !== "string" &&
                    "predicate" in li.solution.solutions[0].expressions[1] &&
                    li.solution.solutions[0].expressions[1]
                        .predicate === namespaces_1.RDF.rest) {
                    ...
                    if (newElt)
                        vals.push(newElt);
                    return rest.object === namespaces_1.RDF.nil
                        ? true
                        : Boolean(chaseList(rest.referenced.type === "ShapeOrResults" // heuristic for `nil OR @<list>` idiom
    ...
    valuesToSchema: function (values) {
        ...
        const v = values;
        const t = values[namespaces_1.RDF.type][0].ldterm;
        if (t === namespaces_1.SX.Schema) {
            /* Schema { "@context":"http://www.w3.org/ns/shex.jsonld"
            ...
            if (namespaces_1.SX.startActs in v)
                ret.startActs = v[namespaces_1.SX.startActs].map((e) => {
                    const ret = {
                        type: "SemAct",
            ...
            if (namespaces_1.SX.imports in v)
                ret.imports = v[namespaces_1.SX.imports].map((e) => {
                    return e.ldterm;
                });
        ...
        function findType(v, elts, f) {
            const t = v[namespaces_1.RDF.type][0].ldterm.substr(namespaces_1.SX._namespace.length);
            const elt = elts[t];
            if (!elt)
                return Missed;
            if (elt.nary) {
                ...
                ret[elt.prop] = v[namespaces_1.SX[elt.prop]].map((e) => {
                    return valueOf(e);
                });
                return ret;
            }
        ...
        function shapeExpr(v) {
            ...
            if (ret !== Missed)
                return ret;
            const t = v[namespaces_1.RDF.type][0].ldterm;
            if (t === namespaces_1.SX.Shape) {
            ...
            else if (t === namespaces_1.SX.NodeConstraint) {
                const ret = { type: "NodeConstraint" };
                if (namespaces_1.SX.values in v)
                    ret.values = v[namespaces_1.SX.values].map((v1) => {
                        return objectValue(v1);
                    });
                if (namespaces_1.SX.nodeKind in v)
                    ret.nodeKind = v[namespaces_1.SX.nodeKind][0].ldterm.substr(namespaces_1.SX._namespace.length);
                [
                    "length",
                    "minlength",
                    "maxlength",
                    ...
                    if (namespaces_1.SX[a] in v)
                        ret[a] = parseFloat(v[namespaces_1.SX[a]][0].ldterm.value);
                });
                if (namespaces_1.SX.pattern in v)
                    ret.pattern = v[namespaces_1.SX.pattern][0].ldterm.value;
                if (namespaces_1.SX.flags in v)
                    ret.flags = v[namespaces_1.SX.flags][0].ldterm.value;
                if (namespaces_1.SX.datatype in v)
                    ret.datatype = v[namespaces_1.SX.datatype][0].ldterm;
                return ret;
            }
            else {
        ...
        function objectValue(v, expectString) {
            if ("nested" in v) {
                const t = v.nested[namespaces_1.RDF.type][0].ldterm;
                if ([namespaces_1.SX.IriStem, namespaces_1.SX.LiteralStem, namespaces_1.SX.LanguageStem].indexOf(t) !== -1) {
                    const ldterm = v.nested[namespaces_1.SX.stem][0].ldterm.value;
                    return {
                        type: t.substr(namespaces_1.SX._namespace.length),
                        stem: ldterm,
                    };
                }
                else if ([namespaces_1.SX.Language].indexOf(t) !== -1) {
                    return {
                        type: "Language",
                        languageTag: v.nested[namespaces_1.SX.languageTag][0].ldterm.value,
                    };
                }
                else if ([namespaces_1.SX.IriStemRange, namespaces_1.SX.LiteralStemRange, namespaces_1.SX.LanguageStemRange].indexOf(t) !== -1) {
                    const st = v.nested[namespaces_1.SX.stem][0];
                    let stem = st;
                    if (typeof st === "object") {
                    ...
                    if (namespaces_1.SX.exclusion in v.nested) {
                        // IriStemRange:
                        ...
                        ret.exclusions = v.nested[namespaces_1.SX.exclusion].map((v1) => {
                            return objectValue(v1, t !== namespaces_1.SX.IriStemRange);
                        });
                    }
                    return ret;
                }
        ...
        function tripleExpr(v) {
            ...
            const t = v[namespaces_1.RDF.type][0].ldterm;
            if (t === namespaces_1.SX.TripleConstraint) {
                const ret = {
                    type: "TripleConstraint",
                    predicate: v[namespaces_1.SX.predicate][0].ldterm,
                };
                ["inverse"].forEach((a) => {
                    if (namespaces_1.SX[a] in v)
                        ret[a] = !!v[namespaces_1.SX[a]][0].ldterm.value;
                });
                if (namespaces_1.SX.valueExpr in v)
                    ret.valueExpr = extend({ id: v[namespaces_1.SX.valueExpr][0].ldterm }, "nested" in v[namespaces_1.SX.valueExpr][0]
                        ? shapeExpr(v[namespaces_1.SX.valueExpr][0].nested)
                        : {});
                minMaxAnnotSemActs(v, ret);
                return ret;
            }
        ...
        function minMaxAnnotSemActs(v, ret) {
            if (namespaces_1.SX.min in v)
                ret.min = parseInt(v[namespaces_1.SX.min][0].ldterm.value);
            if (namespaces_1.SX.max in v) {
                ret.max = parseInt(v[namespaces_1.SX.max][0].ldterm.value);
                if (isNaN(ret.max))
                    ret.max = UNBOUNDED;
            }
            if (namespaces_1.SX.annotation in v)
    ...
    errsToSimple: function (val) {
        ...
        if (val.type === "FailureList") {
            ...
            return ["Unexpected triple(s): {"]
                .concat(val.unexpectedTriples.map((t) => {
                return ("  " +
                    t.subject +
                    " " +
                    t.predicate +
